...
def build_delta_revision_context(
    *,
    ...
    if not selected and cue_count:
        selected = {1}
    neighborhood = {number for number in selected for number in (number - 1, number, number + 1) if 1 <= number <= cue_count}
    ...
    context = {
        ...
        "relevant_groups": [item for item in all_groups if item["group_id"] in group_refs],
        "relevant_presets": [item for item in all_presets if item["reference"] in preset_refs],
        "capability_summary": _compact_capabilities(capability_profiles),
        "song": _compact_song(song_context),
        "spatial": _compact_spatial(spatial_context),
    }
