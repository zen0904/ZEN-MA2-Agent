-- ZEN MA2 Agent read-only state adapter for grandMA2 3.9.x.
-- It only traverses show objects and emits Echo records. It never selects,
-- clears, stores, updates, clones, patches, or executes a MA command.
...
local function number(object)
    local value = safely(gma.show.getobj.number, object)
...
local function group_membership(group_no)
    ...
    for index = 0, amount - 1 do
        ...
        if fixture_no and fixture_no > 0 then fixtures[#fixtures + 1] = tostring(fixture_no) end
    end
...
local function layout(layout_no)
    ...
    for index = 0, amount - 1 do
        local child = safely(gma.show.getobj.child, pool, index)
        local reference = tostring(property(child, "object") or name(child))
        local object_no, kind = reference:match("[Ff]ixture%s+(%d+)"), nil
        ...
        if kind and object_no and x and y then
            local item = "{\"type\":" .. quote(kind) .. ",\"" .. kind .. "\":" .. object_no .. ",\"x\":" .. x .. ",\"y\":" .. y
            local width, height, rotation = tonumber(property(child, "width")), tonumber(property(child, "height")), tonumber(property(child, "rotation"))
            if width then item = item .. ",\"width\":" .. width end
            if height then item = item .. ",\"height\":" .. height end
            if rotation then item = item .. ",\"rotation\":" .. rotation end
            items[#items + 1] = item .. "}"
        end
    end
