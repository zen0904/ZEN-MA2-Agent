"""Assertions for files that must be present in every portable ZEN core bundle."""
...
def assert_portable_resources(bundle: Path) -> list[str]:
    """Fail fast when a portable core bundle is missing required resources."""
    ...
    plugin = next(
        (element for element in root.iter() if element.tag.rsplit("}", 1)[-1] == "Plugin"),
        None,
    )
    ...
    for filename in (
        ...
        "ZEN_REAL_LIGHTING_DESIGN_TEST.json",
    ):
