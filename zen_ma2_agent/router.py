...
class IntentRouter:
    ...
    def route(self, text: str, registry: SkillRegistry) -> RequestRoute:
        ...
        try:
            ...
            return RequestRoute(ResponseType.NEEDS_CLARIFICATION, normalized)
        if intent.kind.startswith("state_") or intent.kind in {"layout_items_query", "layout_all_objects_query", "preset_list", "effect_list", "effect_next_page", "effect_lookup", "sequence_executor_lookup", "page_executor_list"}:
